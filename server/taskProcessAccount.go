...
func updateAccountProcessingCompletion(ctx context.Context, aaId int, db *sql.DB, updateId int64, jobErr, rdsErr error, ec2Err error, historyErr error, riErr error, weekHistoryErr error) {
	updateNextUpdateAccount(db, aaId)
	rErr := registerAccountProcessingCompletion(db, updateId, jobErr, rdsErr, ec2Err, historyErr, riErr, weekHistoryErr)
	if rErr != nil {
		logger := jsonlog.LoggerFromContextOrDefault(ctx)
...
func registerAccountProcessingCompletion(db *sql.DB, updateId int64, jobErr, rdsErr error, ec2Err error, historyErr error, riErr error, weekHistoryErr error) error {
