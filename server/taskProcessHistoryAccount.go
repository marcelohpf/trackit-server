...
func taskProcessHistoryAccount(ctx context.Context) error {
	args := flag.Args()
	logger := jsonlog.LoggerFromContextOrDefault(ctx)
	logger.Debug("Running task 'process-history'.", map[string]interface{}{
		"args":         args,
		"master-email": config.MasterEmail,
	})
	if config.MasterEmail != "" {
		return ingestHistoryDataForAccount(ctx, 0)
	} else if len(args) != 1 {
		return errors.New("taskProcessHistoryAccount requires an integer argument or a Master Trackit Account (master-email).")
	} else if aaId, err := strconv.Atoi(args[0]); err != nil {
		return err
	} else {
		return ingestHistoryDataForAccount(ctx, aaId)
	}
	// return ingestDataForAccount(ctx, 1)
}

// ingestHistoryDataForAccount ingests the AWS api data for an AwsAccount.
func ingestHistoryDataForAccount(ctx context.Context, aaId int) (err error) {
	var tx *sql.Tx
